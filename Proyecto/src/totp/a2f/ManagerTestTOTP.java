...
public class ManagerTestTOTP {
	...
	public void testRfcCompliancy() throws Exception {
		ManagerTOTP totp = new ManagerTOTP(rfcInterval, rfcSize, 1);
		for (int i = 0; i < rfcTime.length; i++) {
			long t = rfcTime[i] * 1000;
			String code = totp.generarOTP(rfcSecret.getBytes(), totp.getTimeInterval(t));
	...
	public void testBackwardsValidation() throws Exception {
		...
		for (long i : itvls) {
			codes.add(manager.generarOTP(secret, i));
		}
	...
	public void testMockBackwardsValidation() throws Exception {
		...
		ManagerTOTP spy = Mockito.spy(new ManagerTOTP(rfcInterval, rfcSize, 1));
